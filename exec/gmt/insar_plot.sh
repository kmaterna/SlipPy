...
awk '{print $1, $2, $3*1000}' $obs_insar_file | gmt psxy -Sc0.04 -Cmycpt.cpt -R$range -J$proj -O -K >> $output

# Model
gmt pscoast -R$range -J$proj -Wthin,black -Di -N1 -N2 -B1.0 -B+t"Model" -X3.5i -Gwhite -Slightgray -O -K >> $output
awk '{print $1, $2, $3*1000}' $model_insar_file | gmt psxy -Sc0.04 -Cmycpt.cpt -R$range -J$proj -O -K >> $output
...
awk '{print $1, $2, $3-$4}' temp_insar.txt | gmt psxy -Sc0.04 -Cmycpt.cpt -R$range -J$proj -O -K >> $output
