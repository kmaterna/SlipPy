...
gmt psscale -Cmycpt.cpt -Dx15c/8c+w5c/0.5c+jTC -Bxaf+l"Vertical" -By+l"mm" -K -O >> $output
...
`echo $ll1 + 0.4 | bc` `echo $ll2 + 0.2 | bc` .005 0 0 0 0 5mm model
EOF
gmt psvelo -A+e+gblack+pthickest -Wblack -Se$horiz_scale/0.68/10 -R$range -J$proj -O -K <<EOF>> $output
`echo $ll1 + 0.4 | bc` `echo $ll2 + 0.3 | bc` .005 0 0.001 0.001 0 5mm obs
EOF
